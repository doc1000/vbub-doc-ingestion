...
# optional whitespace, opening ---, content, closing ---, optional trailing newline.
_YAML_FRONT_MATTER_RE = re.compile(
    r"^\s*---\s*\n.*?\n---\s*\n?",
    re.DOTALL,
)

# Matches a Markdown H1 heading: optional leading whitespace, # , space, text.
_MD_H1_RE = re.compile(r"^\s*#\s+(.+)")


def _decode(file_bytes: bytes) -> str:
    """Decode bytes to str.  Tries UTF-8 (with BOM), then falls back to latin-1."""
    # utf-8-sig strips the BOM if present; pure UTF-8 is also accepted.
    try:
